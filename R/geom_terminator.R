...
GeomTerminator <- ggproto("GeomTerminator", Geom,
        required_aes = c("x", "y"),
        ...
        draw_panel = function(data, panel_scales, coord) {
              ...
              part2 <-  segmentsGrob(x0 = coords$x - coords$h / 3, 
                                     x1 = coords$x + coords$h / 3, 
...
geom_terminator <- function(mapping = NULL, data = NULL, stat = "identity",
                         position = "identity", na.rm = FALSE, 
                         show.legend = NA, inherit.aes = TRUE, ...) {
        ggplot2::layer(
                geom = GeomTerminator, mapping = mapping,  
