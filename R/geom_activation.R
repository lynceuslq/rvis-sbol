...
GeomActivation <- ggproto("GeomActivation", Geom,
        ...
        draw_panel = function(data, panel_scales, coord) {
              ...
              part3 <-  segmentsGrob(x0 = coords$xmax, 
                                    ...
                                    arrow = arrow(angle = 30, length = unit(0.025, "inches"), ends = "last", type = "open") , 
                                    gp = gpar(col = alpha(coords$col, coords$alpha), lwd = coords$lwd)) 
              
             
              
              gTree(children = gList(part1, part2, part3))  
              
        })

geom_activation <- function(mapping = NULL, data = NULL, stat = "identity",
                         position = "identity", na.rm = FALSE, 
                         show.legend = NA, inherit.aes = TRUE, ...) {
        ggplot2::layer(
                geom = GeomActivation, mapping = mapping,  
